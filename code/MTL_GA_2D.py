...
# Flatten a structured x-y-t solution and create train/validation samples.
def build_xyz_split(data, n_train, n_valid, seed=42):
    x = data["x"]  # (Nx,)
    y = data["y"]  # (Ny,)
    t = data["t"]  # (Nt,)
    U = data["U"]  # (Nt, Nx, Ny)

    # Ensure U has shape (Nx, Ny, Nt).
    if U.shape == (len(t), len(x), len(y)):
        U = np.transpose(U, (1, 2, 0))  # (Nx, Ny, Nt)

    # Build the 3D coordinate grid.
    X_grid, Y_grid, T_grid = np.meshgrid(x, y, t, indexing="ij")  # (Nx, Ny, Nt)

    X = np.column_stack([X_grid.ravel(), Y_grid.ravel(), T_grid.ravel()])  # (N, 3)
...
# Split an irregular matrix of coordinates and values into train/validation samples.
def build_matrix_split(data, n_train, n_valid, seed=42):
    X = data[:,0:3] # (N, 3)
    Y_out = data[:,3:4]  # (N, 1)
...
# Compute 2D spatial and temporal derivatives with PyTorch autograd.
def compute_derivatives_2d(u, X, LHS="ut"):
    # X: (N,3) -> [x, y, t]
    ...
    if LHS == "ut":
        return u_t, u_x, u_xx, u_xxx, u_y, u_yy, u_yyy
    else:
        return u_tt, u_x, u_xx, u_xxx, u_y, u_yy, u_yyy
...
# Randomly sample one 2D candidate PDE term under derivative constraints.
def sample_gene_module_2d(
    rng,
    base_genes=BASE_GENES_2D,
    min_len=1,
    max_len=4,
    allow_repeat=True,
    p=[1, 1, 0.5, 0.1],
    ...
    # Choose the derivative direction: x or y.
...
def sample_genome(rng, n_modules=None, min_modules=2, max_modules=6, **module_kwargs):
    if n_modules is None:
        n_modules = int(rng.integers(min_modules, max_modules + 1))

    raw = [sample_gene_module_2d(rng, **module_kwargs) for _ in range(n_modules)]
...
# Convert a 2D genome into a callable PDE feature library.
def genome_to_library_2d(genome):
    term_names = ["*".join(g) for g in genome]

    def library_fn(u, u_x, u_xx, u_y, u_yy, u_xxx=None, u_yyy=None):
        gene_map = {
            "u": u,
            "ux": u_x,
            "uxx": u_xx,
...
# Train one 2D PINN for a selected PDE library.
def train_pinn_single_case_2d(
    model,
    X_np, U_np,
    LHS,
    seed,
    ...
    """
    2D PDE loss on random collocation points sampled from (x,y,t) domain.
    ...
    if t_range is None:
        t_range = (float(X_np[:, 2].min()), float(X_np[:, 2].max()))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # mask = (torch.abs(u_c) >= 1e-1).reshape(-1)
        # X_c = X_c[mask]
        # u_c = model(X_c).reshape(-1, 1)
        #
        # print(X_c.shape[0])
        # Filtering can leave no collocation points; keep this guard if filtering is re-enabled.
        # if X_c.shape[0] == 0:
        #     raise RuntimeError("X_c is empty.")

        Left_u_t, u_x, u_xx, u_xxx, u_y, u_yy, u_yyy = compute_derivatives_2d(u_c, X_c,LHS=LHS)

        # The library function below must use the 2D derivative set.
        if "Wave" in name:
            Phi = torch.cat([u_xx ,u_yy], dim=1)
        elif "Pollutant" in name:
            Phi = torch.cat([u_xx, u_yy,u_x,u_y], dim=1)
        elif "Poisson" in name:
            Phi = torch.cat([u_xx, u_yy], dim=1)

        # Use u_t as the default left-hand side.
...
# Train multicase 2D models while estimating shared PDE coefficients.
def train_multicase_competitive_2d(
    models,
    ...
    for key in case_keys:
        ...
        data_cache.append((key, X_t, U_t, (x_min, x_max, y_min, y_max, t_min, t_max)))
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for model, (case_id, X_data, U_data, dom) in zip(models, data_cache):
            ...
            x_min, x_max, y_min, y_max, t_min, t_max = dom
            ...
            colloc_cache.append((Left_u_t, u_x, u_xx, u_xxx, u_y, u_yy, u_yyy, u_c))
        ...
        for (Left_u_t, u_x, u_xx, u_xxx, u_y, u_yy, u_yyy, u_c) in colloc_cache:
            lib_out = library_fn(u_c, u_x, u_xx, u_y, u_yy, u_xxx, u_yyy)
...
def fitness(
    ...
    library_fn, term_names = genome_to_library_2d(genome)
    result = train_multicase_competitive_2d(
        models=models,
        cases=cases,
        prefix=prefix,
        LHS=LHS,
...
def mutate_genome(rng, genome, p_mut=0.3, min_len=1, max_len=3):
    ...
    if choice == 0 and len(g) > 1:
        ...
        g.append(sample_gene_module_2d(rng, min_len=min_len, max_len=max_len))
    else:
        idx = int(rng.integers(0, len(g)))
        g[idx] = sample_gene_module_2d(rng, min_len=min_len, max_len=max_len)
    return g
