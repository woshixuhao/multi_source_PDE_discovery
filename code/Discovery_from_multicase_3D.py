# Run workflow:
# 1. Select a 3D dataset by editing `prefix`, then load all available cases.
...
prefix = 'Heat_3D'  # Dataset family to load.
n_train=5000  # Number of training samples per case.
n_valid=5000  # Number of validation samples per case.
noise_level=0  # Noise percentage; keep 0 for clean data.
mode='Discover'  # Workflow stage: Discover, Valid, or PIC_select.
case_index=7  # Reserved for single-case workflows.
...
if prefix=='Heat_3D':
    ...
    LHS='ut'
    # Load one 3D heat MATLAB case and return solution and grids.
    def load_heat3d_case(mat_path: str):
        """
        Read MATLAB output 'heat3d_block.mat' from the 3D heat solver.

        Expected variables in .mat:
          - x, y, z: 1D grids
          - t_save: 1D time grid (saved/subsampled)
          - T_save: temperature array, shape (Nt_save, Nx, Ny, Nz)

        Returns:
          u: np.ndarray  (Nt, Nx, Ny, Nz)  (temperature, renamed as u)
          x: np.ndarray  (Nx,)
          y: np.ndarray  (Ny,)
          z: np.ndarray  (Nz,)
          t: np.ndarray  (Nt,)
        """
        # Try scipy first; if it's a v7.3 MAT (HDF5) fall back to h5py.
        try:
            from scipy.io import loadmat
            md = loadmat(mat_path)
            loader = "scipy"
        ...
        def _squeeze_vec(a):
            return np.asarray(a).squeeze().astype(np.float64)

        if loader == "scipy":
            # v7 / non-HDF5 MAT
            if "u_save" not in md:
                raise KeyError(f"Missing 'u_save' in {mat_path}. Keys={list(md.keys())}")

            u = np.asarray(md["u_save"], dtype=np.float64)
            x = _squeeze_vec(md["x"])
            y = _squeeze_vec(md["y"])
            z = _squeeze_vec(md["z"])
            t = _squeeze_vec(md["t_save"])
            return u, x, y, z, t


    dataset = {}
    data_dir = "data/Heat_3D"  # Update this path for your local data folder.
    prefix = "Heat3D_IC"  # Expected files: Heat3D_IC1.mat ... Heat3D_IC7.mat.
    for i in range(1, 8):
        ...
        try:
            U, x, y,z, t = load_heat3d_case(path)
        ...
        dataset[key] = {"U": U, "t": t, "x": x, "y": y,"z":z}
        # Print the loaded array shapes for quick sanity checking.
        print(t)
        print(f"[ok] loaded {path}: U={U.shape}, x={x.shape}, t={t.shape}, y={y.shape}, z={z.shape}")
...
split_dataset = {}
for key, data in dataset.items():
    X, Y, X_train, Y_train, X_valid, Y_valid = build_xyzt_split(
...
if mode == 'PIC_select':
    ...
    library_fn, term_names = genome_to_library_3d(best_genome)
    models, _ = train_multicase_competitive_3d(
        models=models,
        cases=split_dataset,
        prefix=prefix,
        LHS=LHS,
    ...
    for key in case_keys:
        ...
        data_cache.append((x_min, x_max, y_min, y_max, z_min, z_max, t_min, t_max))
    ...
    for case_idx, key in enumerate(case_keys):
        ...
        x_min, x_max, y_min, y_max, z_min, z_max, t_min, t_max = data_cache[case_idx]

        x_c = rng.uniform(x_min, x_max, size=(1000, 1))
        y_c = rng.uniform(y_min, y_max, size=(1000, 1))
        z_c = rng.uniform(z_min, z_max, size=(1000, 1))
    ...
    for step, term_name in enumerate(sorted_term_names, start=1):
        ...
        library_fn, _ = genome_to_library_3d(current_genome)
        metrics = train_multicase_competitive_3d(
            models=models,
            cases=split_dataset,
            prefix=prefix,
            LHS=LHS,
    ...
    library_fn, term_names = genome_to_library_3d(best_genome_final)
    models, lines = train_multicase_competitive_3d(
        models=models,
        cases=split_dataset,
        prefix=prefix,
        LHS=LHS,
